...
# Path to Wheat_Leaf_Rust folder
BASE_DIR = os.path.dirname(__file__)
LEAF_RUST_DIR = os.path.join(BASE_DIR, 'cropDiseaseDataset', 'Wheat_Leaf_Rust')

# Valid image extensions
VALID_EXTS = ('.png', '.jpg', '.jpeg', '.jfif', '.bmp')

print("=" * 70)
print("🔄 Renaming Wheat_Leaf_Rust images to consistent format")
print("=" * 70)

if not os.path.exists(LEAF_RUST_DIR):
    print(f"❌ Directory not found: {LEAF_RUST_DIR}")
    exit(1)

# Get all image files
image_files = []
for f in os.listdir(LEAF_RUST_DIR):
...
print(f"\n📁 Found {len(image_files)} images in {LEAF_RUST_DIR}")
print(f"📝 Will rename to format: leaf_rust_00001.jpg, leaf_rust_00002.jpg, etc.\n")
...
for idx, old_name in enumerate(image_files, start=1):
    old_path = os.path.join(LEAF_RUST_DIR, old_name)
    ...
    temp_path = os.path.join(LEAF_RUST_DIR, temp_name)
...
for temp_name, idx, ext in temp_files:
    temp_path = os.path.join(LEAF_RUST_DIR, temp_name)
    new_name = f"leaf_rust_{idx:05d}{ext}"
    new_path = os.path.join(LEAF_RUST_DIR, new_name)
...
print(f"   • New format: leaf_rust_00001{ext} to leaf_rust_{success_count:05d}{ext}")
print("=" * 70)

# Save rename log
log_path = os.path.join(BASE_DIR, 'rename_log_leaf_rust.txt')
with open(log_path, 'w', encoding='utf-8') as f:
    f.write(f"Renamed {success_count} images in Wheat_Leaf_Rust folder\n")
    f.write("=" * 70 + "\n\n")
