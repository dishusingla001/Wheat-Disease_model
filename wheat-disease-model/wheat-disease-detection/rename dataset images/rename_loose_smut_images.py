...
# Path to Wheat_Loose_Smut folder
BASE_DIR = os.path.dirname(__file__)
LOOSE_SMUT_DIR = os.path.join(BASE_DIR, 'cropDiseaseDataset', 'Wheat_Loose_Smut')

# Valid image extensions
VALID_EXTS = ('.png', '.jpg', '.jpeg', '.jfif', '.bmp')

print("=" * 70)
print("🔄 Renaming Wheat_Loose_Smut images to consistent format")
print("=" * 70)

if not os.path.exists(LOOSE_SMUT_DIR):
    print(f"❌ Directory not found: {LOOSE_SMUT_DIR}")
    exit(1)

# Get all image files
image_files = []
for f in os.listdir(LOOSE_SMUT_DIR):
...
print(f"\n📁 Found {len(image_files)} images in {LOOSE_SMUT_DIR}")
print(f"📝 Will rename to format: loose_smut_00001.jpg, loose_smut_00002.jpg, etc.\n")
...
for idx, old_name in enumerate(image_files, start=1):
    old_path = os.path.join(LOOSE_SMUT_DIR, old_name)
    ...
    temp_path = os.path.join(LOOSE_SMUT_DIR, temp_name)
...
for temp_name, idx, ext in temp_files:
    temp_path = os.path.join(LOOSE_SMUT_DIR, temp_name)
    new_name = f"loose_smut_{idx:05d}{ext}"
    new_path = os.path.join(LOOSE_SMUT_DIR, new_name)
...
print(f"   • New format: loose_smut_00001{ext} to loose_smut_{success_count:05d}{ext}")
print("=" * 70)

# Save rename log
log_path = os.path.join(BASE_DIR, 'rename_log_loose_smut.txt')
with open(log_path, 'w', encoding='utf-8') as f:
    f.write(f"Renamed {success_count} images in Wheat_Loose_Smut folder\n")
    f.write("=" * 70 + "\n\n")
